...
static char	*construct_string(struct s_gnl_buf_list_info *lst_info)
{
	...
	{
		to_write = min(lst_info->head->bytes_read - lst_info->bytes_curr,
...
{
	static struct s_gnl_buf_list_info	lst_info = {
		.head = NULL, .bytes_curr = 0, .bytes_eol = 0};

	read_fd_until_newline(&lst_info, fd);
	if (!lst_info.bytes_eol)
		return (NULL);
	return (construct_string(&lst_info));
}
